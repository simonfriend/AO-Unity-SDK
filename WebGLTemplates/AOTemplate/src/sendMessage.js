...
export async function sendDryrun(pid, data, tagsStr, id, objectCallback, methodCallback) {
    ...
    try {
        ...
        console.error('Error in sendDryrun:', error);
...
export async function sendMessageHyperBeam(pid, data, tagsStr, id, objectCallback, methodCallback, useMainWallet, chain, hyperBeamUrl) {
    ...
    try {
        ...
        // Create HyperBEAM connection using high-level API (same as aoconnect-editor.js)
        const ao = connect({
            MODE: "mainnet",
            URL: hyperBeamUrl,
        ...
        // Send message via HyperBEAM (high-level API)
        console.log("Sending HyperBEAM message with tags:", tags);
        ...
        console.log("HyperBEAM message sent, ID:", messageId);
        ...
        console.log("HyperBEAM result received:", result);

        // Convert result to Unity format
        // Result structure matches legacy format: { Messages, Output, Spawns, Error, ... }
        const response = {
            Messages: result.Messages || [],
            Spawns: result.Spawns || [],
            Output: result.Output || { data: "", print: false },
